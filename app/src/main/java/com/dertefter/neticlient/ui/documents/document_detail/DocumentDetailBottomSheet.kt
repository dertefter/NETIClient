...
@AndroidEntryPoint
class DocumentDetailBottomSheet : BottomSheetDialogFragment() {

    private val documentDetailViewModel: DocumentDetailViewModel by viewModels()


    companion object {
        const val TAG = "DocumentDetailBottomSheet"

        fun newInstance(documentItem: DocumentsItem): DocumentDetailBottomSheet {
            val fragment = DocumentDetailBottomSheet()
    ...
    fun collectCancelStatus(){
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                documentDetailViewModel.cancelSuccess.collect { cancelSuccess ->
    ...
    fun collectCancelable(){
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                documentDetailViewModel.cancelable.collect { cancelable ->
                    binding.cancelClaim.isGone = cancelable != true
                }
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        documentDetailViewModel.updateCancelable(documentItem.number!!)
        ...
        binding.cancelClaim.setOnClickListener {
            documentDetailViewModel.cancelDocument(documentItem.number!!)
        }
